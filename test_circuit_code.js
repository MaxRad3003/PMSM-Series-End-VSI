// Standalone test for renderCircuitCanvas
function getRenderCircuitCode(isHebrew) {
    const titleBus = isHebrew ? '+Vdc (400V מסילת מתח ישר)' : '+Vdc (400V DC Bus Rail)';
    const titleGnd = isHebrew ? '0V (מסילת הארקה / אפס)' : '0V (Ground / Return Rail)';
    const motorLabel = isHebrew ? 'מנוע PMSM (חיבור כוכב Y)' : 'PMSM Motor (Star Y Connection)';
    const neutralLabel = isHebrew ? 'מוליך נייטרל (ענף 4)' : 'Neutral Conductor (Leg 4)';
    const faultLabel = isHebrew ? '⚠️ תקלת ענף 4 (מנותק)' : '⚠️ Leg 4 Fault (Isolated)';
    ...
    function renderCircuitCanvas() {
        ...
        const motorRadius = Math.min(W * 0.14, (botRailY - topRailY) * 0.40);
        ...
        // 3 Stator Coils positions (angles: Phase A = -150 deg, Phase B = 150 deg, Phase C = 0 deg)
        ...
        if (leg4Fault) {
            ...
            ctx.textAlign = 'center';
            ctx.fillText('${faultLabel}', crossX, neutralY - 9 * dpr);
        } else {
            ...
            ctx.textAlign = 'center';
            ctx.fillText('${neutralLabel}', (legX[3] + motorCenterX) / 2, neutralY - 6 * dpr);
        }

        // 4. VOLTAGE POLARITY HUD & TELEMETRY
        // Calculate instantaneous v_an, v_bn, v_cn
        ...
        // Update external DOM labels if present
